...
- Structure: a compact deployment card sits below the YOLO warning and above AI usage. Its single primary action installs or repairs the per-user CC Switch and OpenCode clients, then applies the verified provider and selected model. The user only supplies Base URL and API Key in the existing fields.
- Behavior: pressing the action first verifies the API and model catalog, then runs one idempotent native deployment transaction. Progress advances through verification, client installation, provider import, and final configuration verification. CC Switch's official confirmation surface may appear briefly, but YUME completes only the exact import whose provider name, endpoint, and model match the verified request. No coordinate or cursor automation is used.
