...
- Structure: the Desktop pet tab starts with a compact raised group for pet scale, visibility and a clearly labelled Pet always on top switch, followed by nickname, pack management, mouse-follow, and render tuning. The switch uses the existing alwaysOnTop preference and controls the native pet window.
