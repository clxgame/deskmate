...
### Unified conversation organizer
- The organizer fills the existing chat content area with a raised heading, a sunken metadata search field, Active/Pinned/Archived filters, and a single scrollable conversation list. At compact widths the project/source/date filters wrap; at full width they share a row. No content extends the window horizontally.
- Rows use existing `--s-2`, `--s-3`, `--s-5`, `--r-sm`, `--t-body`, and `--t-small` tokens. Titles lead; project, source, activity time, and capability/availability labels follow. A disclosure exposes rename, pin, archive/restore, and delete. Page size is 50 to bound rendering at large history sizes.
- Loading, empty results, unavailable cached sessions, request errors, pending mutations, inline rename, and delete confirmation are distinct states. Delete confirmation names the conversation and irreversible outcome; archive remains reversible. Destructive controls use `--danger`, offline labels `--warn`, selected filters `--accent-soft` and `--accent-ink`.
- All controls have visible labels and the shared focus ring. The organizer receives keyboard focus on entry, traps focus while open, closes with Escape, and restores prior focus on exit. Delete confirmation initially focuses Cancel. Dark, mint, peach, and lavender inherit the shared palette without component-specific colors or decorative motion.

### Chat message Markdown
- Navigation tick motion: all ticks rest at 12px; rail hover expands them to 16px; the hovered tick reaches 20px. Fixed left origin with scaleX transitions over 180ms, easing cubic-bezier(0.22, 1, 0.36, 1), lets adjacent ticks smoothly exchange lengths. Current reading position changes color only. Keyboard focus-visible receives the same expansion; reduced-motion disables transitions. Leaving the rail restores the equal resting lengths.
- Conversation navigation follows the supplied Codex rail reference: two or more user turns create a left-edge 28px rail, 16px hit-row height and 2px muted ticks; current ticks use the text token; hover/focus lengths follow the navigation motion rule. Reserve 36px at the message list edge. A 16px-radius raised-surface preview appears beside the rail, clamped inside the timeline, with two lines of question and three lines of reply summary. Hover/focus previews, activation jumps, Escape dismisses. Long rails scroll independently. While reading older turns, streaming does not pull the list to the bottom; scrolling back down or sending a new question resumes following. All four themes use shared color and typography tokens.
- Code cards follow the supplied Codex screenshot: one continuous sunken surface, subtle border, 20px radius and 16px horizontal padding. No header divider. Left header shows a 16px code icon and fence language (fallback `text`); right header uses a transparent 28px copy icon button. Success becomes a check for two seconds; failure becomes a warning with retry tooltip; polite status remains visually hidden so feedback never shifts layout. Header stays outside horizontal scrolling; copied text preserves code whitespace. Existing theme surface, text, line and focus tokens adapt the reference to all four themes.
- Assistant replies use CommonMark plus GFM tables, task lists and strikethrough. User messages and empty-state copy remain literal text. Only authored emphasis is bold; no automatic keyword emphasis. Original message strings remain unchanged for history and memory.
- Typography: body `--t-body` 13px, `--chat-line-height` 1.7; headings use `--t-head` 15px and `--t-title` 14px rather than document-sized headings. Paragraph/block gap `--s-3`, list-item gap `--s-1`, list indent `--s-5`. First/last blocks have no extra outer gap.
- Layout: assistant bubbles may use 92% of the list width; user bubbles keep 85%. Every flex child can shrink. Inline code wraps long paths; fenced code keeps exact whitespace and scrolls horizontally inside its surface. Tables scroll inside a labelled keyboard-focusable region, never widen the window. Both use the existing sunken surface, line, small radius and spacing tokens.
- Links use accent-ink, underline and visible keyboard focus. HTTP(S)/mailto open separately; internal footnote anchors stay in the current document. Other URLs are inert. Raw HTML is not executed. Markdown image syntax shows alternative text without loading remote images; explicit attachments retain their own previews.
- Streaming: only the current assistant message gets incomplete-format completion for presentation, with math completion disabled. Finished/stopped/history messages parse their original source. Memoized completed messages and stable Markdown node keys avoid remounting existing code/table scroll containers as text grows. No entrance animation or per-token live-region announcements.
...
### Settings row
- A separate “聊天大界面” General switch defaults off and persists independently. Chat dimensions are 420×560 / 720×760 logical pixels, clamped to the pet monitor work area using the same 16px gutter. Existing chat anchoring uses the resulting native size; typography and chat controls stay unchanged. Restore chat size at startup and before opening it.
- General settings offer a persisted “大界面” switch, off by default. Native settings dimensions are 720×520 logical pixels when off and 1040×760 when on, clamped to the current monitor work area with a 16px gutter and centred within it. Typography, sidebar and controls retain their existing tokens; additional area goes to the scrollable content. Restore the selected size at startup and whenever settings are opened.
...
### Work journal periods and tasks
- Report content reuses the safe, themed Markdown renderer from chat for headings, emphasis, lists, tables, quotations and code. Existing reports open in preview; a labelled Edit/Preview button switches to the original Markdown textarea without changing the draft. New reports start in edit mode. Tables and code scroll within the content region, not the page; save, copy and export retain the Markdown source. Both daily and weekly reports share this behavior.
- Daily and weekly reports are the primary views. Task lists and task actions sit below the selected period's reports; report schedules use a secondary action. Generic entry opens select the daily view and the entry's business date.
- Daily selects one business date. Weekly defaults to the current ISO week, Monday through Sunday, using the existing 03:00 business-day boundary. A compact disclosure shows the selected year and `第37周（9.7-9.13）`; expanding it opens a single-column wheel centred on the viewed week. A neutral `--surface-hover` band marks the centre; the actual current week independently uses `--accent-ink` and a short localized current-week label. Past weeks use `--text-dim` and remain interactive; future weeks use `--text`. Rows have no individual border or card background.
- The wheel uses local `--worklog-week-row-height` (36px) and `--worklog-week-visible-rows` (5) tokens: a 180px viewport with two rows of empty padding at each end. First and last weeks can therefore occupy the centre. Native vertical scroll snapping handles mouse wheel, trackpad and touch; pointer dragging supplements mouse input. Only a centred, settled selection updates reports and tasks, once per changed week. Edge masking fades outer rows without masking the focus outline; the centre and adjacent rows stay readable. Reduced motion uses immediate alignment. Full localized week/date labels remain on one line at narrow widget widths.
- The disclosure stays open after selection; its trigger, outside clicks and Escape dismiss it. Closing cancels uncommitted movement; reopening centres the last committed week. The entry retains the selected ISO year; the expanded panel contains only the week wheel, with no year toolbar or return-to-current-week action. A single listbox tab stop exposes active/selected options and current-date semantics. Arrow keys move one week without wrapping, Home/End reach year boundaries, Enter aligns and selects then dismisses, and Escape restores trigger focus. Outside dismissal preserves the destination control's focus. Listeners, timers and pointer capture are scoped to the open wheel and cleaned on dismissal, hiding or unmount.

### Widget selector
- Structure: a compact grid of labelled 88px square buttons with centred 32px functional icons, using the existing quiet surface treatment. Scheduled tasks, Pomodoro and Work journal (工作日志) are three peer widgets selecting one configuration region. Pet always-on-top belongs to Desktop pet settings, not the widget page. Work journal contains the complete entries, daily reports, weekly reports and report schedules workspace, with no independent sidebar category or master enable switch.
- Behavior: scheduled-task drafts survive switching widgets. Work journal mounts only after its first visit, then retains filters and editor drafts while inactive; hidden content and popovers cannot receive focus. Receipt links and legacy worklog events select Widgets → Work journal, including repeated entity requests. A generic opening returns to the daily report and tasks and clears obsolete detail targets; selecting a peer tile preserves the current draft instead.
...
- Work journal period selection uses a single full-width daily date or weekly picker, with no project filter. Queries include all projects in the selected period. Retain existing spacing/control tokens and keep complete dates readable beside the native calendar icon at narrow widths.
...
- Structure: the Desktop pet tab starts with a compact raised group for pet scale, visibility, Pet always on top, then mouse-follow for GLB pets, followed by nickname and pack management. The always-on-top switch controls the native pet window. Outline width, rim range/intensity and specular intensity controls are omitted; their stored rendering preferences remain in use.
...
- Outline width, rim-light width/intensity and toon specular remain stored rendering preferences; their controls are omitted from Desktop pet settings. Switching personas reapplies the saved tuning to the active glb-viewer toon shader.

### AI providers and usage cards
- Model selection keeps the native select and its existing theme/focus tokens. Non-selectable optgroup headings read `provider · manufacturer`, ordered first by configured provider then known manufacturer; only populated groups appear, with a localized other-manufacturers group last. Manufacturer inference uses recognized model ID families, then display names for opaque IDs; ambiguous names remain in Other. Options preserve exact sidecar/model IDs, original display names and routing behavior. Default remains the first selectable option.
- Provider disclosure buttons show text and arrows on transparent backgrounds with transparent borders, including hover and expanded states. The shared outer container remains visible. Hover and expansion use accent text; keyboard focus retains the existing visible outline.
...
GIF desktop pets use the supplied transparent 240×240 images in a square, bottom-centred display inside the existing pet footprint. Per-action scale and vertical offset preserve authored close-up/full-body framing. Seven native images preload before display; loading or switching personas immediately removes the previous image and hit region. GIF pets expose shared scale, chat, dragging and settings controls; mouse-follow, shader tuning and poke are absent. A load failure uses the themed alert surface and an actionable Settings button.
...
### Native workbench theme inheritance
- The OpenCode workbench retains its own layout and components, but YUME Settings is the sole source for its four color choices: `dark`, `mint`, `peach`, and `lavender`. The workbench does not introduce a fifth palette or a separate persisted choice.
- `src/theme.css` owns the palette values. Workbench build output derives the same semantic tokens, then maps OpenCode background, raised surface, text, border, focus, status, and accent variables to them. The mapping covers the shell, sidebar, search, composer, menus, and dialogs; no color is selected by screen coordinates or screenshot matching.
- The native bridge exposes only the current theme ID to the workbench. It applies the saved choice at startup and listens for theme-only change events, so an open workbench changes without reload. Unknown IDs fall back to `dark`; the dark palette sets a dark color scheme and the three pastel palettes set a light one.
- Check all four palettes in the real workbench at the normal window size and a narrow size, including a popover or dialog and visible keyboard focus. Text remains legible over every themed surface.
...
- Chat approvals show the requested operation and complete scrollable parameters, with Allow once and Cancel actions. Workspace command requests that provide a narrow reusable pattern also show Always allow this kind, followed by the exact saved pattern and workspace scope. Error state uses --danger; disabled actions prevent repeat submission. Unknown tools are denied; no inactive future capability controls are shown.
- Tool permissions settings list remembered workspace command rules below the category controls. Each row shows the exact pattern and workspace path with a single Revoke action; an empty state replaces the list when no rules are saved.

### Conversation deletion memory option
The existing optional deletion of conversation-only memories remains checked by default inside the permanent-delete confirmation. Use the inherited small text, spacing, focus and native checkbox styles; disabled state follows mutation progress. Unchecking preserves memories. This setting never applies to archive.
