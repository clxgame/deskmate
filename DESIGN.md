...
- Daily selects one business date. Weekly defaults to the current ISO week, Monday through Sunday, using the existing 03:00 business-day boundary. A compact disclosure shows the selected year and `第37周（9.7-9.13）`; expanding it exposes all weeks of that ISO year with year navigation. Past weeks use `--text-dim` and stay interactive; the current week uses `--accent-soft` and `--accent-ink`; future weeks use ordinary button colors. Selection uses a strong border independently of current-week styling, and keyboard focus uses the existing focus ring.
- Week options form a vertically scrollable group limited to `--worklog-week-list-height` (176px), so the report and task content stay close. Use existing spacing, control, surface and typography tokens; the list fits narrow widget widths without horizontal overflow.
