...
use super::{CcSwitchInstallation, CcSwitchPlatformError};
use crate::ccswitch::protocol::SecretImportUrl;

#[cfg(windows)]
mod readiness;

#[cfg(windows)]
use readiness::wait_for_cc_switch_frontend;

pub(super) const WINDOWS_PROTOCOL_KEY: &str = r"HKEY_CLASSES_ROOT\ccswitch\shell\open\command";
pub(super) const CC_SWITCH_EXE: &str = "CC-Switch.exe";
#[cfg(windows)]
...
pub(super) const WINDOWS_FILE_PROTOCOL_HANDLER_ARG: &str = "url.dll,FileProtocolHandler";
const WINDOWS_SYSTEM_ROOT: &str = r"C:\Windows";

#[cfg(windows)]
pub(super) fn detect_system_installation() -> Result<CcSwitchInstallation, CcSwitchPlatformError> {
...
#[cfg(windows)]
pub(super) fn prepare_system_import(
    ...
    wait_for_cc_switch_frontend(&mut child)
}

#[cfg(windows)]
pub(super) fn open_system_url(url: &SecretImportUrl) -> Result<(), CcSwitchPlatformError> {
...
#[cfg(windows)]
pub(super) fn read_packaged_version(executable: &Path) -> Option<String> {
    read_packaged_version_with_product_reader(executable, read_executable_product_version)
}

#[cfg(windows)]
pub(super) fn read_packaged_version_with_product_reader(
    executable: &Path,
...
pub(super) fn detect_installation_from_registry_output(
...
pub(super) fn parse_registered_executable(command: &str) -> Option<PathBuf> {
...
pub(super) struct UrlOpenCommand {
    pub program: PathBuf,
    pub handler_arg: &'static str,
}

impl UrlOpenCommand {
    pub(super) fn args_for<'a>(&self, url: &'a SecretImportUrl) -> [&'a str; 2] {
...
pub(super) fn trusted_system_url_open_command_from_root(
    system_root: &Path,
...
pub(super) fn build_url_open_command(
    system_root: &Path,
) -> Result<UrlOpenCommand, CcSwitchPlatformError> {
