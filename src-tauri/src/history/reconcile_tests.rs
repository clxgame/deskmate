use super::{tests::{serve, session}, NativeHistoryClient};
use crate::history::{catalog::CatalogStore, catalog_import::new_entry, catalog_model::*, reconcile};
...
fn pending_store() -> (std::path::PathBuf, CatalogStore) {
    ...
    let mut row = new_entry(CatalogIdentity::Native {
        sidecar_id: SIDECAR_ID.into(), directory: "c:/fixture".into(), session_id: "ses_retry".into(),
    }, "Retry".into(), ConversationSource::LightChat, 1, 2);
    row.tombstone = Some(DeletionTombstone { requested_at: 3, remote_deleted: false });
    store.update(|rows| { rows.push(row); Ok(()) }).unwrap();
    (root, store)
}
...
fn pending_delete_does_not_contact_mutation_when_native_task_is_busy() {
    ...
    assert!(errors.is_empty(), "unexpected remote deletion attempt: {errors:?}");
    assert!(!store.all().unwrap()[0].tombstone.as_ref().unwrap().remote_deleted);
...
fn delete_retry_survives_remote_failure_and_confirms_only_observed_removal() {
    ...
    let (url, worker) = serve(vec![
        (200, format!("[{row}]")), (200, "{}".into()), (200, row.clone()), (503, "{}".into()),
        (200, format!("[{row}]")), (200, "{}".into()), (200, row), (200, "true".into()), (404, "{}".into()),
    ]);
    let client = NativeHistoryClient::new(&url, "Basic fixture").unwrap();
    ...
    assert!(!reopened.all().unwrap()[0].tombstone.as_ref().unwrap().remote_deleted);
