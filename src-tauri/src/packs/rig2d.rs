...
pub(super) fn parse_config(bytes: &[u8]) -> Result<Figure, String> {
    ...
    if figure.schema_version != 1
        || figure.renderer != "silver-cat-v1"
        || figure.canvas.width != 512
        || figure.canvas.height != 512
        || figure.states.len() != STATES.len()
        || STATES
            .iter()
            .any(|state| !figure.states.contains_key(*state))
    {
        return Err("rig2d schema, renderer or states invalid".into());
    }
    for (name, state) in &figure.states {
        let texture_count = match name.as_str() {
            "sleep" => 1,
            "talking" => 3,
            _ => 2,
        };
        let rig = &state.rig;
        if state.textures.len() != texture_count
            || state.textures.iter().any(|path| !safe_png_path(path))
            || rig
                .neck
                .iter()
                .chain(&rig.eyes)
                .chain(&rig.mouth)
                .any(|v| !bounded(*v, 0.0, 512.0))
            || rig
                .eye_radius
                .iter()
                .chain(&rig.mouth[2..])
                .any(|v| !bounded(*v, 0.0, 512.0) || *v == 0.0)
        ...
        for row in state.alignment.blink.iter().chain(&state.alignment.mouth) {
            if !bounded(row[0], -4.0, 4.0)
                || !bounded(row[1], -4.0, 4.0)
                || !bounded(row[2], -512.0, 512.0)
            {
...
pub(super) fn validate(root: &Path) -> Result<(), String> {
    fs::read_to_string(root.join("persona.md"))
        .map_err(|error| format!("rig2d persona.md missing: {error}"))?;
...
pub(super) fn validate_png(bytes: &[u8]) -> Result<(), String> {
    ...
    let mut reader = decoder
        .read_info()
        .map_err(|error| format!("rig2d PNG invalid: {error}"))?;
    ...
    reader
        ...
        .map_err(|error| format!("rig2d PNG decode failed: {error}"))?;
    reader
        .finish()
        .map_err(|error| format!("rig2d PNG truncated: {error}"))?;
    Ok(())
}
