...
fn config() -> serde_json::Value {
    serde_json::from_str(include_str!(
        "../../../public/personas/baobao/figure-rig2d.json"
    ))
    .unwrap()
}
...
fn shared_rig2d_contract_matches_frontend_and_authoring() {
    let fixtures: serde_json::Value =
        serde_json::from_str(include_str!("../../../tests/fixtures/rig2d-contract.json")).unwrap();
...
fn imports_all_authored_rig2d_assets_and_preserves_previous_pack_on_bad_upgrade() {
    ...
    assert_eq!(
        fs::read(root.join("packs/baobao/pack.json")).unwrap(),
        previous
    );
...
fn rig2d_config_accepts_authored_data_and_rejects_unsafe_parameters() {
    ...
    for (pointer, invalid) in [
        ...
        *input.pointer_mut(pointer).unwrap() = invalid;
        assert!(
            super::rig2d::parse_config(&serde_json::to_vec(&input).unwrap()).is_err(),
            "{pointer}"
        );
    }
}
...
fn rig2d_png_checks_crc_and_full_decode() {
    let source =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../public/personas/baobao/assets/sleep-0.png");
...
fn rig2d_import_rejects_missing_and_corrupt_textures_without_replacing_old_pack() {
    ...
    for texture in [None, Some(b"not a png".as_slice())] {
        let mut entries = vec![
            (
                "pack.json",
                br#"{"packId":"baobao","personas":[{"id":"baobao","renderType":"rig2d"}]}"#
                    .as_slice(),
            ),
            ("personas/baobao/persona.md", b"new prompt".as_slice()),
            ("personas/baobao/figure-rig2d.json", config.as_slice()),
        ];
        if let Some(bytes) = texture {
            entries.push(("personas/baobao/assets/idle-0.png", bytes));
        }
