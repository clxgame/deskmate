...
export type GifDisplayState = GifAnimationState | "sleep";
type Animations<T> = Readonly<Record<GifAnimationState, T>> & { readonly sleep?: T };
...
function parseV1(input: unknown): Figure2dConfigV1 {
  ...
  const source = object(root.animations, "animations", [...GIF_ANIMATION_STATES, "sleep"]);
  function animation(state: GifDisplayState): Figure2dAnimationV1 {
  ...
  return {
    ...
    animations: { idle: animation("idle"), thinking: animation("thinking"), working: animation("working"), talking: animation("talking"), success: animation("success"), error: animation("error"), leaving: animation("leaving"), ...(Object.hasOwn(source, "sleep") ? { sleep: animation("sleep") } : {}) },
...
function parseV2(root: Record<string, unknown>): Figure2dConfigV2 {
  ...
  const source = object(root.animations, "animations", [...GIF_ANIMATION_STATES, "sleep"]);
  function animation(state: GifDisplayState): Figure2dAnimationV2 {
  ...
  const animations = { idle: animation("idle"), thinking: animation("thinking"), working: animation("working"), talking: animation("talking"), success: animation("success"), error: animation("error"), leaving: animation("leaving"), ...(Object.hasOwn(source, "sleep") ? { sleep: animation("sleep") } : {}) };
