...
export const initialGifState: GifState = {
  request: null, terminal: false, base: "idle", thinkingSince: null, feedback: null,
...
export function reduceGifState(
  ...
  timing: GifTiming = defaultGifTiming,
): GifState {
  ...
  if (event.type === "start") {
    ...
    return { request: event, terminal: false, base: "thinking", thinkingSince: now, feedback: null,
...
export function gifDisplayState(state: GifState, now: number, timing: GifTiming = defaultGifTiming): GifAnimationState {
  if (state.feedback !== null && now < state.feedback.until) return state.feedback.state;
  if (state.base === "thinking" && state.thinkingSince !== null && now - state.thinkingSince >= timing.thinkingEscalationMs) return "working";
  return state.base;
}

export function gifNextDeadline(state: GifState, now: number, timing: GifTiming): number | null {
  if (state.feedback !== null && state.feedback.until > now) return state.feedback.until;
  if (state.thinkingSince !== null && state.thinkingSince + timing.thinkingEscalationMs > now) return state.thinkingSince + timing.thinkingEscalationMs;
  return null;
}
